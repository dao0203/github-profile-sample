...
class HomeViewModel(
    getContributionsForThePastMonthUseCase: GetContributionsForThePastMonthUseCase
) : ViewModel() {

    val uiState = getContributionsForThePastMonthUseCase("dao0203").map {
        HomeUiState(isLoading = false, contributions = it)
    }.catch {
        emit(HomeUiState(isLoading = false, error = it))
    }.stateIn(
